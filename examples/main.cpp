...
class MyLandProcessor : public IEventProcessor
{
   ...
   public:
    MyLandProcessor(LandId id, std::string name)
    ...
    {
        std::cout << "Land [" << name_ << " (" << id_
                  << ")] processing event from God " << event.sourceGodId
                  << ". Info: '" << event.info << "'"
        ...
        std::this_thread::sleep_for(
            std::chrono::milliseconds(10 + (rand() % 50)));
    }
...
int main()
{
    ...
    auto landUnderworld =
        std::make_shared<MyLandProcessor>(UNDERWORLD_ID, "Underworld");
