...
/**
 * @brief Manages event dispatching between Gods (sources) and Lands
 * (processors).
 */
class EldenTree final
{
   public:
     ...
     * @param processor A shared pointer to the object implementing
     * IEventProcessor.
     * @throws std::runtime_error if a processor with the same LandId already
     * exists.
     */
   ...
   private:
    ...
    std::unordered_map<LandId, std::unique_ptr<ThreadSafeQueue<GodEvent>>>
        _eventQueues;
    std::mutex _queuesMutex;  // Protects access to the _eventQueues map itself
                              // (not individual queues)
