...
// --- Constructor ---
EldenTree::EldenTree(unsigned int numThreads)
    : _numThreads(numThreads == 0 ? std::thread::hardware_concurrency()
                                  : numThreads),
      _stopRequested(false)
{
    if (_numThreads == 0) _numThreads = 1;

    std::cout << "Starting EldenTree with " << _numThreads
...
void EldenTree::registerProcessor(std::shared_ptr<IEventProcessor> processor)
{
    ...
    {
        throw std::runtime_error("Processor with LandId " +
                                 std::to_string(landId) +
                                 " already registered.");
    }
    ...
    _eventQueues[landId] =
        std::make_unique<ThreadSafeQueue<GodEvent>>(_stopRequested);

    std::cout << "Processor for Land " << landId << " registered successfully."
              << std::endl;
}
...
void EldenTree::unregisterProcessor(LandId landId)
{
    ...
    {
        std::cout << "Processor for Land " << landId << " unregistered."
                  << std::endl;
    }
    else
    {
        std::cerr << "Warning: Attempted to unregister non-existent processor "
                     "for Land "
                  << landId << std::endl;
    }
}
...
void EldenTree::postEvent(GodEvent event) noexcept
{
    ...
    if (it != _eventQueues.end())
    {
        ...
        {
            // Should not happen
            std::cerr << "Error: Queue pointer is null for Land "
                      << event.targetLandId << std::endl;
        }
    }
    else
    {
        lock.unlock();  // Ensure lock is released
        std::cerr << "Warning: No registered processor for target Land "
                  << event.targetLandId << ". Event discarded." << std::endl;
    }
}
...
void EldenTree::stop()
{
    ...
    bool alreadyStopping =
        _stopRequested.exchange(true, std::memory_order_acq_rel);
    ...
    while (!(_stopRequested.load(std::memory_order_acquire)))
    {
        ...
        for (LandId currentLandId : landIdsToCheck)
        {
            ...
            if (queuePtr)
            {
                ...
                {
                    // Successfully got an event (or woken by stop, but queue
                    // wasn't empty)
                    eventFound = true;
            ...
            if (processor)
            {
                ...
                catch (const std::exception &e)
                {
                    std::cerr << "Worker thread [" << std::this_thread::get_id()
                              << "]: Exception during processEvent for Land "
                              << processedLandId << ": " << e.what()
                              << std::endl;
                    // Decide how to handle processor errors (e.g., log, disable
                    // processor?)
                }
                catch (...)
                {
                    std::cerr
                        ...
                        << "]: Unknown exception during processEvent for Land "
                        << processedLandId << std::endl;
                }
            }
            else
            {
                std::cerr << "Warning: Worker thread ["
                          << std::this_thread::get_id()
                          << "]: Processor for Land " << processedLandId
                          << " disappeared before event processing."
                          << std::endl;
            }
        }
        else if (!_stopRequested.load(std::memory_order_acquire))
        {
            // No event found across all queues, wait a short time before
            // checking again This prevents busy-looping when idle.
